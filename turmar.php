			...
			<table class="table table-hover" name='tblList'>
				...
				<tbody>
					<?php foreach($mascotas as $mascota){ ?>
						...
						<!--MARCACIONES-->
							<?php
								if ($mascota->marcaciones == NULL ) {
									echo '<td style="background-color: #F59696 ">'.$mascota->marcaciones.'</td>';
								} elseif ($mascota->marcaciones <> 4 ) {
									echo '<td style="background-color: #F7d9d9 ">'.$mascota->marcaciones.'</td>';
								} else {
						...
						<!--//HORA DE ENTRADA-->
							<?php
								if ($mascota->Entrada == NULL ) {
									echo '<td style="background-color: #F59696 ">'.$mascota->horaentrada.'</td>';
								} elseif ($mascota->Entrada < -5) {
									echo '<td style="background-color:#F7d9d9">'.$mascota->horaentrada.'</td>';
								} elseif ($mascota->Entrada < 30) {
									echo '<td>'.$mascota->horaentrada.'</td>';
								} else {
									echo '<td  style="background-color: #eeffeb ">'.$mascota->horaentrada.'</td>';
								}
							?>
						<!--//HORA DE SALIDA-->
							<?php
								if ($mascota->Salida == NULL ) {
									echo '<td style="background-color: #F59696 ">'.$mascota->horasalida.'</td>';
								} elseif ($mascota->Salida < -5) {
									echo '<td style="background-color:#F7d9d9">'.$mascota->horasalida.'</td>';
								} elseif ($mascota->Salida < 30) {
									echo '<td>'.$mascota->horasalida.'</td>';
								} else {
									echo '<td  style="background-color: #eeffeb ">'.$mascota->horasalida.'</td>';
								}
							?>
						<!--//HORA DE SALIDA-->
							<?php
								if ($mascota->MINDESCANSO == NULL ) {
									echo '<td style="background-color: #F59696 ">'.$mascota->DESCANSO.'</td>';
								} elseif ($mascota->MINDESCANSO > $mascota->intMinutosDescanso) {
									echo '<td style="background-color:#F7d9d9">'.$mascota->DESCANSO.'</td>';
								} else {
