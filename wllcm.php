...
{
        /*$sentencia1 = $base_de_datos->query("SELECT TOP (1) [PUNTAJE]
            FROM [Analitica].[dbo].[vw_conteos_monthpoints]
            where ANIO=year(getdate()) and MES=MONTH(GETDATE())
            and bodega='". $_SESSION['username'] ."'" 
                );
        $PUNTAJE = $sentencia1->fetchObject();
        $xpuntaje = $PUNTAJE->PUNTAJE;
    */
