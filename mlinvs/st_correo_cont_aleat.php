<?php
    ...
    ini_set('display_errors', '0'); // evitar que se impriman por consola
    ini_set('log_errors', '1');
    ini_set('error_log', '/storage/tareas/st_correo_cont_aleat.log');
    use PHPMailer\PHPMailer\PHPMailer;
    use PHPMailer\PHPMailer\SMTP;
    use PHPMailer\PHPMailer\Exception;
    require_once('/storage/sc22/vendor/autoload.php');
    $contraseña = "Datos.22";
    $usuario = "consultas";
    $nombreBaseDeDatos = "STORECONTROL";
    $rutaServidor = "10.10.100.12";
    try{
        $db = new PDO("sqlsrv:server=$rutaServidor;database=$nombreBaseDeDatos;TrustServerCertificate=yes;", $usuario, $contraseña);
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);    
        $s11 = $db->query("SELECT  a.cod_almacen, a.nombre
        ...
        $s1 = $db->query("SELECT s.id as id_cab, FK_ID_almacen as id_alm,  [date] as fec,
        ...
        function ZendMail($Xcode, $Xnome, $Xmail, $Xfec, $XmailSuper){
            try {         
                ...
                $mail->AddCC("acolumba@mabeltrading.com.ec");
                ...
                if(!$mail->send()){ file_put_contents('/storage/tareas/st_correo_cont_aleat.log', "Mailer Error: " . $mail->ErrorInfo . "\n", FILE_APPEND); }
                else{ file_put_contents('/storage/tareas/st_correo_cont_aleat.log', "éxito\n", FILE_APPEND); }
            }catch (Exception $e){
                file_put_contents('/storage/tareas/st_correo_cont_aleat.log', "Error al enviar correo a {$xmail}: {$mail->ErrorInfo}\n", FILE_APPEND);
            }
        }
        ...
        function ZendMailNovedades($Xcode, $Xnome, $Xmail, $Xfec, $XmailSuper, $id_cab){ 
            try {        
                ...
                $nombreBaseDeDatos = "STORECONTROL";
                $rutaServidor = "10.10.100.12";
                $dba = new PDO("sqlsrv:server=$rutaServidor;database=$nombreBaseDeDatos;TrustServerCertificate=yes", $usuario, $contraseña);
        ...
        foreach($regs as $row){ ZendMail($row->cod_almacen,$row->nombre,$row->email,$row->fec,$row->emailSuper); } 
        foreach($novedades as $row){ ZendMailNovedades($row->cod_almacen,$row->nombre,$row->email,$row->fec,$row->emailSuper,$row->id_cab); }
    }catch(Exception $e){ file_put_contents('/storage/tareas/st_correo_cont_aleat.log', "Error en conexión: " . $e->getMessage() . "\n", FILE_APPEND); }
?>
