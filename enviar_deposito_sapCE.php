...
try {
    // Obtener depósito
    $stmt = $db->prepare("SELECT * FROM DepositosTiendasCE WHERE id = ?");
    $stmt->execute([$id]);
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'http://192.168.2.12:8086/api/Deposit/CE',
...
if ($httpCode === 200 || $httpCode === 201) {
    try {
        $upd = $db->prepare("UPDATE DepositosTiendasCE SET creadoSAP = 1 WHERE id = ?");
        $upd->execute([$id]);
