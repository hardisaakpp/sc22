...
if ($accion === "guardar_local") {
    ...
    // ❗ Verificar duplicado
    $verificar = $db->prepare("SELECT COUNT(*) FROM DepositosTiendasCE WHERE DepositAccount = ? AND U_Ref_Bancar = ?");
...
if ($DepositDate < $fechaMinima || $DepositDate > $fechaActual) {
    ...
    $stmt = $db->prepare("
        INSERT INTO DepositosTiendasCE (
    ...
    if ($ok) {
        echo "<script>
            ...
            window.location.href = '../depDce.php?fecha=" . urlencode($U_Fecha) . "&whsCode=" . urlencode($U_WhsCode) . "';
        </script>";
    } else {
