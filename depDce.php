...
include_once "header.php";
 ...
 $id = $whsCode.$fecha;
    ...
    $sql1 = "
        ...
        JOIN CuentaFinancieraCE c ON d.DepositAccount = c.AcctCode
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped">
                        ...
                        <?php foreach ($depositos as $d): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="php/eliminar_depositoCE.php?id=<?= $d->Id ?>&U_Fecha=<?= urlencode($fecha) ?>&U_WhsCode=<?= urlencode($whsCode) ?>"
                   ...
                   <?php if ((isset($_GET['pflag']) && $_GET['pflag'] == 1)): ?>
                    ...
                    <?php else: ?>
                        <a href="depLce.php" class="btn btn-outline-secondary mt-3">🔙 Volver al resumen</a>
                        <a href="depCce.php?U_Fecha=<?= $fecha ?>" class="btn btn-primary mt-3">📄 Nuevo</a>
...
function cerrarModalIntegracion() {
    ...
    function eliminarRegistro(id) {
        if (confirm("¿Está seguro de eliminar este registro?")) {
            window.location.href = "depDce.php?eliminar=" + id;
        }
    }
    ...
    function integrarDeposito(whsCode, fecha, total) {
        if (confirm(`¿Estás seguro de integrar el depósito del ${fecha} (${whsCode}) por $${total}?`)) {
            fetch('enviar_deposito_sapCE.php', {
                method: 'POST',
