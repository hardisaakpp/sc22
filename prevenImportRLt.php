...
if (isset($_POST['submit'])) {
    ...
    if(isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
        ...
        if (!empty($sheetData)) {
           ...
           foreach ($distinctLOCAL as $i => $value) {
            if (($distinctLOCAL[$i])!=$userName) {
                echo 'Registro ' . $i+2 . ' - ERROR, SOLO PUEDE SUBIR REGISTROS PARA EL LOCAL '.$userName.' <br>';
                $errors++;
            }                
            elseif (empty($distinctLOCAL[$i])) {
                echo 'Registro ' . $i+2 . ' - ERROR, COD LOCAL VACIO <br>';
                $errors++;
            }
        }
           ...
           //validaciones etapa 1 formatos
            foreach ($distinctANIO as $i => $value) {
                ...
                elseif (empty($distinctANIO[$i])) {
                    ...
                    $errors++;
                }elseif ($distinctANIO[$i]>2025 OR $distinctANIO[$i]<2023) {
                    echo 'Registro ' . $i+2 . ' - ERROR, EL AÑO ESTA FUERA DEL RANGO PERMITIDO (2023-2025) <br>';
                    $errors++;
                }
            }
            ...
            foreach ($distinctMES as $i => $value) {
                ...
                elseif (empty($distinctMES[$i])) {
                    ...
                    $errors++;
                }elseif ($distinctMES[$i]<>12 && $distinctMES[$i]<>1) {
                    echo 'Registro ' . $i+2 . ' - ERROR, EL VALOR MES ESTA FUERA DEL RANGO PERMITIDO (DICIEMBRE-ENERO) <br>';
                    $errors++;
                }
            }
            ...
            if ($errors>0) {
                ...
                <a href="prevenImpRLt.php" class="btn btn-warning">OK, VOLVER</a>';
            } else {
                ...
                //------------------------------------------------------------
                   ...
                   for ($i=1; $i<count($sheetData); $i++) {
                    ...
                    <a href="prevenLRLt.php" class="btn btn-warning">OK, VOLVER</a>';
