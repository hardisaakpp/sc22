...
SELECT c.Id, c.NumTransferencia, sum(d.CantidadAbierta) as Cantidad, SUM(D.Scan) AS Escaneados

  FROM [dbo].[TransferenciasCabecera] c join TransferenciasDetalle d on c.Id=d.id_TrCab
  where c.CreadaTransferencia=0 and d.BodegaDestino='".$almacenTran."'
  GROUP BY c.Id, c.NumTransferencia
                    ...
                    <tbody>
                       <?php foreach ($resumen as $r): ?>
                            <tr>
                                ...
                                <td><?= $r->Escaneados ?></td>
                                <td>
                                    <a class="btn btn-primary btn-sm" href="recP.php?idcab=<?= $r->Id ?>">
